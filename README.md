# 🚀 王往AI 公众号工作流 v5.0 (Performance & Stability Edition)

一套完整的 AI 驱动的微信公众号内容生产工作流，针对**新号起量**阶段优化，从选题到发布**全流程自动化**。

## ✨ 功能特性 (v5.0)

### 新增亮点
- 🚀 **极速扫描**：`TrendingDiscoverer` 采用并发模型，全网信号探测速度提升 300%。
- 📱 **微信生态优先**：全面移除知乎源，深度集成**微信公众号热榜**与社交舆情，内容更接地气。
- 💎 **多级搜索降级**：Perplexity (深度) -> Tavily (广度) -> Exa (兜底) 三级防御，确保永不空手。
- 🛡️ **UI 稳定性增强**：重构 Streamlit 状态管理，彻底解决按钮失效、重复点击导致的逻辑冲突。
- 🧹 **代码净化**：全局优化正则作用域，彻底消除 `local variable referenced before assignment` 隐患。
- 🎯 **Final 决策 v5.0**：输出单一最终选题（非 3 选题），定向优先 + 领域权重 + 人设加固。
- 📝 **仿写模式**：`hunt -i "爆款文章.html"` 分析参考文章，自动提取主题后执行定向搜索（支持 HTML/MD/TXT）。

| 模块 | 功能 | 核心策略 |
|------|------|--------|
| 🎯 **选题雷达** | 全网扫描热点，三级容错机制 (Jina/RSS/Tavily) | **微信热榜优先** + **并发加速** + **严格去重** |
| 🔬 **自动研究** | Perplexity 深度摘要 + 多源聚合搜索 | **Perplexity 优先** + **反套壳** |
| ✍️ **写作智能体** | 读取研究笔记，生成硬核技术流初稿 | **专家验证约束** (Anti-Wrapper) |
| 🎨 **排版智能体** | 自动转换为微信风格 HTML，代码高亮 | **多风格排版** (green/blue/orange/minimal/purple/**livid/vue/typewriter**) |
