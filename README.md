# 🚀 王往AI 公众号工作流 v4.7 (Traffic Storm Edition)
...
### 新增亮点
- 🌪️ **流量风暴模式 (TRAFFIC_STORM)**：针对平台推荐算法优化，文风更具社交冲击力，开头即 Hook。
- 📡 **即时信号前移**：`TrendingDiscoverer` 接入 Product Hunt, Hacker News, V2EX 实时榜单，抢占全网 AI 第一波热点。
- ✍️ **爆款标题实验室**：强制生成 5 个维度（反直觉、利益、恐惧、数字、社交货币）的标题，提升点击率。
- 🛠️ **一键行动清单**：文末自动生成结构化实操指令，提升收藏与转发率。
- ⚖️ **决策权重修复**：修正了选题决策逻辑，确保“最新热点报告”具备最高决策权重，防止旧闻干扰。
- 🛡️ **数据安全**：自动备份机制，每次写入前生成 `.bak`（已增加自动清理逻辑）。
- 🧪 **API 节流模式**：支持 `--dry-run`（即将上线），在不消耗 API 额度的情况下验证流程。

| 模块 | 功能 | 核心策略 |
|------|------|--------|
| 🎯 **选题雷达** | 全网扫描热点，三级容错机制 (Jina/RSS/Tavily) | **大厂/开源优先** + **信源净化 (Anti-SEO)** + **严格去重 / 语义去重防饥饿** |
